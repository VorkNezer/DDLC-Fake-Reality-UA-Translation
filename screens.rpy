...
init -501 screen navigation():
    ...
    vbox:
        ...
        if main_menu:
            ...
            if persistent.playgame == 1 and renpy.random.randint(0,3)==3:
                textbutton _("ŔŗñĮ¼»ŧþŀÂŻŕěōì«") action If(persistent.playername, true=Start(), false=Show(screen="name_input", message="Ведіть {i}ваше{/i} ім'я", ok_action=Function(FinishEnterName)))
            else:
                textbutton _("Нова гра") action If(persistent.playername, true=Start(), false=Show(screen="name_input", message="Ведіть {i}ваше{/i} ім'я", ok_action=Function(FinishEnterName)))
        ...
        if _in_replay:
            textbutton _("Завершити повтор") action EndReplay(confirm=True)
        ...
        if renpy.variant("pc"):
            ...
            textbutton _("Допомога") action Help("README.html")
...
init -501 screen main_menu() tag menu:
    ...
    elif persistent.playgame == 1:
        add "menu_art_s_ghost"
        add "menu_art_m"

    if persistent.playgame == 3 or persistent.playgame == 4:
        add "menu_bg"
        add "menu_art_y"

        add "menu_art_n"

        if persistent.playgame == 3:
            add "menu_art_s"
        elif persistent.playgame == 4:
            add "menu_art_s_ghost"
...
init -501 screen about() tag menu:
    ...
    use game_menu(_("Об игре"), scroll="viewport"):
        ...
        vbox:
            ...
            text _("Зроблено за допомогою {a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only].\n\n[renpy.license!t]")
            ...
            grid gui.file_slot_cols gui.file_slot_rows:
                ...
                for i in range(gui.file_slot_cols * gui.file_slot_rows):
                    ...
                    button:
                        ...
                        text FileTime(slot, format=_("{#file_time}%A, %d %B %Y, %H:%M"), empty=_("Пустий слот")):
        ...
        vbox:
            ...
            hbox:
                ...
                if renpy.variant("pc"):
                    ...
                    vbox:
                        ...
                        textbutton _("Повний екран") action Preference("display", "fullscreen")
                if config.developer:
                    vbox:
                        style_prefix "radio"
                        label _("Сторона прокрутки")
                        textbutton _("Відключено") action Preference("rollback side", "disable")
                        textbutton _("Зліва") action Preference("rollback side", "left")
                        textbutton _("Справа") action Preference("rollback side", "right")

                vbox:
                    style_prefix "check"
                    label _("Пропуск тексту")
                    textbutton _("Непрочитане") action Preference("skip", "toggle")
            ...
            hbox:
                ...
                vbox:
                    ...
                    label _("Швидкість виводу тексу")


                    bar value FieldValue(_preferences, "text_cps", range=180, max_is_zero=False, style="slider", offset=20)

                    label _("Затримка при авточитанні")
...
init -501 screen name_input(message, ok_action):
    ...
    frame:
        ...
        input default "" value VariableInputValue("player") length 12 allow "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyzАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЬЫЪЭЮЯабвгдеёжзийклмнопрстуфхцчшщьыъэюя1234567890()-=!@#$%^&*()_+ "
...
init -501 screen yes_no(message, yes_action, no_action):
...
init -501 screen command_imput(ok_action, message):
    modal True
    zorder 200
    ...
    frame:
        ...
        input default "" value VariableInputValue("command_variable") length 50 allow "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz._/\"\'1234567890()"
